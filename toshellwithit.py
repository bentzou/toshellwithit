...
class ToShellWithIt(object):
   def __init__(self, tool_class, help=True):
      ...
      if help:
         self.commands["help"] = Command(self.print_help, [], {"command": None})
   ...
   def _get_commands_dict(self, tool_class):
      ...
      for method_name in tool_class.__dict__.keys():
         ...
         if ismethod(method) and not method_name.startswith("_"):
            req_args, opt_args = self._get_command_args(tool_class, method_name)
   ...
   def _get_command_args(self, tool_class, command):
      ...
      if argspec.defaults:
         ...
         opt_args = OrderedDict(zip(argspec.args[-num_kw_args:], argspec.defaults))
      else:
   ...
   def usage(self):
      ...
      for command, command_info in sorted(self.commands.items()):
         if command == "help":
            ...
            opt_args_str = " " + " ".join(["[" + opt_arg + "]" for opt_arg in opt_args.keys()])
